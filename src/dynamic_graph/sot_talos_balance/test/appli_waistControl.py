...
robot.contactLF = MetaTaskKine6d(
    "contactLF", robot.rdynamic, "LF", robot.OperationalPointsMap["left-ankle"]
)
robot.contactLF.feature.frame("desired")
robot.contactLF.gain.setConstant(1.0)
plug(robot.wp.footLeftDes, robot.contactLF.featureDes.position)  # .errorIN?
locals()["contactLF"] = robot.contactLF

robot.contactRF = MetaTaskKine6d(
    "contactRF", robot.rdynamic, "RF", robot.OperationalPointsMap["right-ankle"]
)
robot.contactRF.feature.frame("desired")
robot.contactRF.gain.setConstant(1.0)
...
robot.taskCom.task.controlGain.value = 1.0
# robot.taskCom.feature.selec.value = '011'

# --- Waist
robot.keepWaist = MetaTaskKine6d(
    "keepWaist", robot.rdynamic, "WT", robot.OperationalPointsMap["waist"]
)
robot.keepWaist.feature.frame("desired")
robot.keepWaist.gain.setConstant(1.0)
