...
comDes_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.dcm_control.name") + "-dcmDes.dat"
)
# estimated CoM (not directly employed)
comEst_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.cdc_estimator.name") + "-c.dat"
)
# reference CoM
comRef_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.com_admittance_control.name") + "-comRef.dat"
)
comSOT_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.dynamic.name") + "-com.dat"
)  # resulting SOT CoM

dcmDes_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.dcm_control.name") + "-dcmDes.dat"
)  # desired DCM
dcmEst_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.estimator.name") + "-dcm.dat"
)  # estimated DCM

zmpDes_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.dcm_control.name") + "-zmpDes.dat"
)  # desired ZMP
zmpSOT_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.dynamic.name") + "-zmp.dat"
)  # SOT ZMP
zmpEst_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.zmp_estimator.name") + "-zmp.dat"
)  # estimated ZMP
zmpRef_data = np.loadtxt(
    "/tmp/dg_" + evalCommandClient("robot.dcm_control.name") + "-zmpRef.dat"
)  # reference ZMP
...
plt.title("ZMP")
plt.legend(["Desired x", "SOT x", "Reference x", "Desired y", "SOT y", "Reference y"])
