...
import dynamic_graph.sot_talos_balance.talos.control_manager_conf as controlManagerConf
import dynamic_graph.sot_talos_balance.talos.ft_calibration_conf as forceConf
...
wp.waist.value = (
    robot.dynamic.WT.value
)  # waist receives a full homogeneous matrix, but only the rotational part is controlled
...
# --- Left foot
robot.leftAnkleController = create_ankle_admittance_controller(
    [0, 0], robot, "left", "leftController"
)

# --- Right foot
robot.rightAnkleController = create_ankle_admittance_controller(
    [0, 0], robot, "right", "rightController"
)
