...
from dynamic_graph.sot_talos_balance.meta_task_pose import MetaTaskPose
...
controller.dfzAdmittance.value = 0.0
...
robot.device.before.addSignal(
    robot.contactLF.feature.name + ".faNufafbDes"
)  # temporary hack to force recomputation
robot.contactLF.task.setWithDerivative(True)
locals()["contactLF"] = robot.contactLF

robot.contactRF = MetaTaskPose(
...
robot.device.before.addSignal(
    robot.contactRF.feature.name + ".faNufafbDes"
)  # temporary hack to force recomputation
...
create_topic(
    ...
    robot=robot,
    data_type="vector",
)  # reference surface right wrench
create_topic(
