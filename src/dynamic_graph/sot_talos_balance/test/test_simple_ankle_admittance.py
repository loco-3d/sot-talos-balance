...
if c:
    runCommandClient(
        "robot.rightPitchAnkleController.tauDes.value = "
        "robot.device.ptorque.value[RightPitchJoint]"
    )
    runCommandClient(
        "robot.leftPitchAnkleController.tauDes.value = "
        "robot.device.ptorque.value[LeftPitchJoint]"
    )
    runCommandClient(
        "robot.rightRollAnkleController.tauDes.value = "
        "robot.device.ptorque.value[RightRollJoint]"
    )
    runCommandClient(
        "robot.leftRollAnkleController.tauDes.value = "
    ...
    if c2:
        ...
        runCommandClient(
            "robot.rightPitchAnkleController.tauDes.value = [-2.5164434873140853]"
        )
        runCommandClient(
            "robot.leftPitchAnkleController.tauDes.value = [-2.5164434873140853]"
        )
        runCommandClient("robot.rightRollAnkleController.tauDes.value = [0.0]")
        runCommandClient("robot.leftRollAnkleController.tauDes.value = [0.0]")
