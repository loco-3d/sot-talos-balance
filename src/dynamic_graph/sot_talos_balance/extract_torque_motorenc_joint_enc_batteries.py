...
info_dict = yaml.load(
    subprocess.Popen(
        ["rosbag", "info", "--yaml", bagname], stdout=subprocess.PIPE
    ).communicate()[0]
)
        ...
        for jn in jointnames:
            ...
            afile.write(
                " ".join(
                    str(m)
                    for m in [
                        messages[jn].motorenc,
                        messages[jn].jointenc,
                        messages[jn].torquesensor,
                        messages[jn].current,
                        diff_secs,
                    ]
                )
        ...
        f.write(
            " ".join(
                ...
                for m in [
                    aps.message.charge,
                    aps.message.current,
                    aps.message.input_voltage,
                    aps.message.battery_voltage,
                    aps.message.regulator_voltage,
                    aps.message.lower_body_voltage,
                    aps.message.charger_voltage,
                    aps.message.upper_body_voltage,
                    diff_secs,
                ]
            )
            + "."
