...
def apply_force(force, duration, body_name="talos::torso_2_link"):
    ...
    apply_body_wrench_proxy = rospy.ServiceProxy(
    ...
    apply_body_wrench_proxy(
        body_name=body_name, wrench=wrench, duration=rospy.Duration(duration)
    )
...
class GazeboLinkStatePublisher(threading.Thread):
    """Utility class reading the state of a given link from Gazebo and publishing."""
    ...
    def run(self):
        ...
        while (not self.stopped()) and (not rospy.is_shutdown()):
            link_state_msg = get_link_state(
                link_name=self.link_name, reference_frame=self.reference_frame
            )
            ...
            velocity = vec2list(link_state.twist.linear) + vec2list(
                link_state.twist.angular
            )
