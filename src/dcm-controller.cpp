...
#define PROFILE_DCMCONTROLLER_ZMPREF_COMPUTATION \
  "DcmController: zmpRef computation                      "
#define PROFILE_DCMCONTROLLER_WRENCHREF_COMPUTATION \
  "DcmController: wrenchRef computation                   "
...
DcmController::DcmController(const std::string& name)
    : Entity(name),
      ...
      CONSTRUCT_SIGNAL_OUT(wrenchRef, dynamicgraph::Vector,
                           m_zmpRefSOUT << m_comSIN << m_omegaSIN << m_massSIN)
  ...
  /* Commands. */
  addCommand("init", makeCommandVoid1(*this, &DcmController::init,
                                      docCommandVoid1("Initialize the entity.",
                                                      "time step")));
  addCommand(
      "resetDcmIntegralError",
      makeCommandVoid0(*this, &DcmController::resetDcmIntegralError,
...
DEFINE_SIGNAL_OUT_FUNCTION(zmpRef, dynamicgraph::Vector) {
  ...
  Eigen::Vector3d zmpRef =
      zmpDes -
      (Eigen::Vector3d::Constant(1.0) + Kp / omega).cwiseProduct(dcmError) -
